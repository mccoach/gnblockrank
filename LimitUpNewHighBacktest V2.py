...
DEFAULT_EXCLUDE_GAIN_LOOKBACK_DAYS = 12
...
def describe_selection_conditions(limit_up_threshold):
    return "\n".join([
        f"1. 第一日涨停：第一日 PCT_CHG >= {float(limit_up_threshold):g}%。",
        "2. 第二日最高价严格高于第一日最高价。",
        "3. 第二日最低价严格高于第一日最低价。",
        "4. 第二日收盘价严格高于第二日开盘价。",
        "5. 第三日最高价严格高于第二日最高价。",
        "6. 第三日收盘价严格高于第三日开盘价。",
        "7. 涨幅排除窗口：第一日前过去10个交易日、第二日、第三日均不得出现 PCT_CHG > 18%；PCT_CHG 按当日收盘价相对前一交易日收盘价计算。",
        "8. 跌幅排除窗口：第一日前过去120个交易日内不得出现 PCT_CHG <= -9%。",
        "9. 剔除 ST：个股名称中包含 ST 的标的全部排除。",
        "10. 剔除次新股：从上市日，即第一条日线日期，往后推一整年作为扫描起点；上市第一年内的日线不参与入选特征扫描。",
        "11. 日期范围口径：以第三日作为入选日，要求第三日落在用户指定日期范围内；第一日、第二日允许早于开始日期，但不能晚于结束日期。",
        "12. 入选日定义：满足上述全部条件后，以第三日作为入选日。",
    ])
...
def describe_selection_conditions(limit_up_threshold):
    return "\n".join([
        f"1. 第一日涨停：PCT_CHG >= {float(limit_up_threshold):g}%。",
        "2. 第二日最高价严格高于第一日最高价。",
        "3. 第二日最低价严格高于第一日最低价。",
        "4. 第二日收盘价严格高于第二日开盘价。",
        "5. 第三日最高价严格高于第二日最高价。",
...
def has_gain_above_threshold_around_selected_pattern(
    rows_all,
    limit_index,
    selected_index,
    lookback_days,
    ...
    """
    判断三日入选结构对应的涨幅排除窗口内，是否出现超过阈值的涨幅。

    业务规则：
    - 第一日为涨停日，本身是入选结构的核心条件，不参与 18% 涨幅排除；
    - 排除窗口由原来的“第一日前过去 10 个交易日”调整为：
      “围绕第三日倒推扩展后的 12 个检查点”，实质覆盖：
        1. 第一日前过去 10 个交易日；
        2. 第二日；
        3. 第三日；
    - 涨幅按当日收盘价相对前一交易日收盘价计算，即 PCT_CHG；
    - 要求窗口内所有参与检查日的 PCT_CHG <= pct_threshold；
    - 只要出现 PCT_CHG > pct_threshold，即判定不符合入选条件。

    注意：
    - 这里显式排除第一日涨停日，否则第一日涨停本身通常会超过 18%，
      会与“第一日涨停”这个核心入选条件自相矛盾；
    - 如果历史不足，则按已有历史检查。
    """
    history_days = int(lookback_days) - 2
    start = max(0, int(limit_index) - history_days)

    check_indexes = list(range(start, int(limit_index)))
    check_indexes.extend([int(limit_index) + 1, int(selected_index)])

    for k in check_indexes:
        if k < 0 or k >= len(rows_all):
            continue
...
def backtest_one_symbol(
    ...
    5. 第三天收盘价严格高于第三天开盘价；
       即涨停后连续两天阳线且创新高；
    6. 涨幅排除窗口内没有出现 >18% 的涨幅：
       第一日前过去 10 个交易日、第二日、第三日的 PCT_CHG 均需 <=18%；
    7. 涨停日过去 120 个交易日内没有出现 >=9% 的跌幅；
    8. 剔除 ST，个股名称中不包含 ST；
    9. 剔除次新股：
       从上市日，即第一条日线日期，往后推一整年作为扫描起点，
       无论指定日期范围如何，都排除上市第一年的日线。

    满足上述条件即达成入选特征，以第三天日期为入选日。
    ...
    while scan_start < n - 2:
        ...
        for i in range(scan_start, n - 2):
            ...
            # 条件2：涨幅排除窗口内不得出现 >18% 的涨幅
            # 检查范围：第一日前过去10个交易日 + 第二日 + 第三日。
            if has_gain_above_threshold_around_selected_pattern(
                    rows_all=rows_all,
                    limit_index=i,
                    selected_index=i + 2,
            ...
            # 条件4：第二天最高价严格高于第一天最高价
            if float(row2["HIGH"]) <= float(row1["HIGH"]):
                continue

            # 条件5：第二天最低价严格高于第一天最低价
            if float(row2["LOW"]) <= float(row1["LOW"]):
                continue

            # 条件6：第二天收盘价严格高于第二天开盘价
            if float(row2["CLOSE"]) <= float(row2["OPEN"]):
                continue

            # 条件7：第三天最高价严格高于第二天最高价
            if float(row3["HIGH"]) <= float(row2["HIGH"]):
                continue

            # 条件8：第三天收盘价严格高于第三天开盘价
        ...
        second_low = float(second_row["LOW"])
        ...
        selected_low = float(selected_row["LOW"])
...
def run_backtest(config, output_path, log_func=print):
    ...
    selection_conditions = describe_selection_conditions(limit_up_threshold)
    rehigh_conditions = "\n".join([
        "1. 从入选日下一交易日开始扫描。",
        "2. 第一次出现某交易日收盘价严格高于入选日收盘价，即记录为再创新高。",
        "3. 每个入选特征点只记录第一次再创新高。",
        "4. 找到一次即停止。",
        "5. 如果截至结束日期仍未找到，则再创新高间隔记为0。",
    ])
...
def style_params_sheet(ws):
    ...
    for row_idx in range(1, ws.max_row + 1):
        ...
        if line_count > 1:
            ws.row_dimensions[row_idx].height = min(360, line_count * 18)
...
def export_excel(output_path, result_rows, params):
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        style_params_sheet(writer.sheets["参数及结果"])
