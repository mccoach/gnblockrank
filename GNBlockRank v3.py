...
UI_BUTTON_WIDTH = 10
UI_MAIN_BUTTON_WIDTH = 10
UI_FIELD_BUTTON_WIDTH = 10
UI_SORT_BUTTON_WIDTH = 4

UI_COMPACT_PAD_X = 4
UI_COMPACT_PAD_Y = 2
UI_SECTION_PAD_Y = 2
UI_ENTRY_PAD_Y = 2
UI_FIELD_CANVAS_HEIGHT = 26

UI_PATH_LABEL_WIDTH = 18
UI_PATH_ENTRY_WIDTH = 120
...
DEFAULT_TDX_SORT_RULES = [
    ...
    {
        "field": "QUALIFIED_RATIO",
        "order": "DESC",
    },
]

DEFAULT_THS_SORT_RULES = [
    {
        "field": "QUALIFIED_COUNT",
        "order": "DESC",
    },
...
def get_selected_field_keys_for_data_requirement(block_fields, detail_fields,
                                                 sort_rules):
    """
    汇总所有会影响计算的数据字段：
    - 用户选择输出的板块字段
...
def calc_required_stock_lookback_records(block_fields, detail_fields,
                                         sort_rules):
    """
...
def block_day_file_path(tdx_root, block_code):
    block_code = str(block_code or "").strip()

    if not re.fullmatch(r"\d{6}", block_code):
        return ""
...
def concept_source_supports_block_index(source_type):
    return source_type == CONCEPT_SOURCE_TDX


def get_available_block_field_defs(source_type):
    if concept_source_supports_block_index(source_type):
        return BLOCK_FIELD_DEFS

    return [
        item for item in BLOCK_FIELD_DEFS
        if item["key"] not in TDX_ONLY_BLOCK_FIELD_KEYS
    ]


def get_available_sort_field_defs(source_type):
    if concept_source_supports_block_index(source_type):
        return SORT_FIELD_DEFS

    return [
        item for item in SORT_FIELD_DEFS
        if item["key"] not in TDX_ONLY_SORT_FIELD_KEYS
    ]


def remove_tdx_only_block_fields(keys, source_type):
    keys = list(keys or [])

    if concept_source_supports_block_index(source_type):
        return keys

    return [key for key in keys if key not in TDX_ONLY_BLOCK_FIELD_KEYS]


def remove_tdx_only_sort_rules(rules, source_type):
    rules = list(rules or [])
    ...
    return [
        ...
        and rule.get("field") not in TDX_ONLY_SORT_FIELD_KEYS
    ]


def default_sort_rules_for_source(source_type):
    if concept_source_supports_block_index(source_type):
        return deepcopy(DEFAULT_TDX_SORT_RULES)

    return deepcopy(DEFAULT_THS_SORT_RULES)


def normalize_source_dependent_config(config):
    source_type = config.get("concept_source", {}).get("type",
                                                       CONCEPT_SOURCE_TDX)

    config["block_fields"]["selected"] = remove_tdx_only_block_fields(
        config["block_fields"].get("selected", []),
        source_type,
    )

    config["sort"]["rules"] = remove_tdx_only_sort_rules(
        config.get("sort", {}).get("rules", []),
        source_type,
    )

    if not config["sort"]["rules"]:
        config["sort"]["rules"] = default_sort_rules_for_source(source_type)

    if not config["block_fields"]["selected"] and not config["detail_fields"][
            "selected"]:
        config["block_fields"]["selected"] = [
            x["key"] for x in get_available_block_field_defs(source_type)
        ]
...
def make_default_config():
    ...
    return {
        "config_version": 3,
        "ui": {
            "window_title": "概念板块统计排序助手 v1.5.0-20260615",
        ...
        "sort": {
            "rules": deepcopy(DEFAULT_TDX_SORT_RULES),
        },
    }
...
def normalize_field_config(config, field_config_key, field_defs):
    valid_keys = [x["key"] for x in field_defs]
    valid_set = set(valid_keys)

    field_config = config.setdefault(field_config_key, {})
    order = list(field_config.get("order") or [])
    selected = list(field_config.get("selected") or [])

    if set(order) != valid_set or len(order) != len(valid_keys):
        order = valid_keys

    selected = [key for key in selected if key in valid_set]

    if not selected:
        selected = valid_keys

    field_config["order"] = order
    field_config["selected"] = selected


def normalize_sort_config(config):
    source_type = config.get("concept_source", {}).get("type",
                                                       CONCEPT_SOURCE_TDX)

    valid_fields = {x["key"] for x in SORT_FIELD_DEFS}
    valid_orders = {x["key"] for x in SORT_ORDER_OPTIONS}

    sort_config = config.setdefault("sort", {})
    rules = sort_config.get("rules")

    if not isinstance(rules, list):
        sort_config["rules"] = default_sort_rules_for_source(source_type)
        return
    ...
    cleaned = remove_tdx_only_sort_rules(cleaned, source_type)

    if not cleaned:
        cleaned = default_sort_rules_for_source(source_type)

    sort_config["rules"] = cleaned


def normalize_config_paths(config):
    config["daily_source"]["tdx_root"] = normalize_user_path(
        config["daily_source"].get("tdx_root", ""),
        True,
    )
    config["concept_source"]["tdx_root"] = normalize_user_path(
        config["concept_source"].get("tdx_root", ""),
        True,
    )
    config["concept_source"]["ths_root"] = normalize_user_path(
        config["concept_source"].get("ths_root", ""),
        True,
    )
    ...
    normalize_field_config(config, "block_fields", BLOCK_FIELD_DEFS)
    normalize_field_config(config, "detail_fields", DETAIL_FIELD_DEFS)
    normalize_sort_config(config)
    normalize_source_dependent_config(config)
...
def save_app_config(config):
    saved = {
        ...
        "output": {
            ...
            "keep_all_blocks":
            bool(config["output"]["keep_all_blocks"]),
            "max_blocks_per_sheet":
            int(config["output"]["max_blocks_per_sheet"]),
            "skip_zero_blocks":
            bool(config["output"]["skip_zero_blocks"]),
            "auto_open_file":
...
def parse_day_records_reverse_for_range(file_path,
                                        start_int,
                                        end_int,
                                        pre_start_records=1):
    """
...
def build_daily_stock_cache(tdx_root,
                            symbols,
                            name_map,
                            start_date,
                            end_date,
...
def build_block_index_cache(tdx_root,
                            blocks,
                            start_date,
                            end_date,
                            log_func,
    ...
    BLOCK_PCT_CHG 单位是百分数，例如 2.35 表示 2.35%。
    """
    start_int = int(start_date)
    end_int = int(end_date)
    ...
    for idx, block in enumerate(blocks, start=1):
        ...
        path = block_day_file_path(tdx_root, block_code)
        ...
        for i in range(1, len(rows)):
            ...
            if i >= 3:
                ...
                if d3_close > 0:
                    block_3d_pct_chg = (float(row["CLOSE"]) -
                                        d3_close) / d3_close * 100.0
        ...
        if idx % 100 == 0:
            log_func(f"已解析板块指数日线：{idx}/{total}，有效板块：{valid_count}")

    log_func(f"板块指数日线解析完成：板块数 {total}，有效板块 {valid_count}，"
             f"缺失/无效 {missing_count}，日期记录 {row_count}")

    return result


def build_block_member_volume_cache(blocks,
                                    daily_all,
                                    output_start_date=None,
                                    output_end_date=None):
    """
    按日期、板块统计成分股总成交量、总成交额，并计算收盘量比。
    ...
    for yyyymmdd in dates:
        ...
        for block in blocks:
            ...
            for symbol_key in block.get("symbols", []):
                ...
                try:
                    total_amount += float(stock.get("AMOUNT", 0) or 0)
                except Exception:
                    pass

            raw_volume.setdefault(yyyymmdd, {})[block_code] = total_volume
            raw_amount.setdefault(yyyymmdd, {})[block_code] = total_amount
    ...
    for idx, yyyymmdd in enumerate(dates):
        ...
        for block in blocks:
            ...
            current_amount = raw_amount.get(yyyymmdd, {}).get(block_code, 0.0)

            recent_dates = dates[max(0, idx - 5):idx]
            recent_volumes = [
                raw_volume.get(d, {}).get(block_code, 0) for d in recent_dates
            ]

            valid_recent_volumes = [v for v in recent_volumes if v > 0]

            if valid_recent_volumes:
                avg_volume = sum(valid_recent_volumes) / len(
...
def sort_field_title_map():
    return {x["key"]: x["title"] for x in SORT_FIELD_DEFS}


def sort_field_key_by_title(title):
    title = str(title or "").strip()

    for item in SORT_FIELD_DEFS:
        if item["title"] == title:
            return item["key"]

    raise ValueError(f"未知排序字段：{title}")


def sort_order_title_map():
    return {x["key"]: x["title"] for x in SORT_ORDER_OPTIONS}
...
def validate_sort_rules(sort_rules, source_type=CONCEPT_SOURCE_TDX):
    available_fields = {
        x["key"]
        for x in get_available_sort_field_defs(source_type)
    }
    valid_orders = {x["key"] for x in SORT_ORDER_OPTIONS}

    if not isinstance(sort_rules, list):
        raise ValueError("排序条件必须是列表。")

    if not sort_rules:
        raise ValueError("至少需要设置一个排序条件。")

    for rule in sort_rules:
        if not isinstance(rule, dict):
            raise ValueError("排序条件格式错误。")

        field = rule.get("field")
        order = rule.get("order")

        if field not in available_fields:
            raise ValueError(f"当前概念源不支持该排序字段：{field}")

        if order not in valid_orders:
            raise ValueError(f"未知排序方向：{order}")


def sort_block_rank_records(records,
                            sort_rules,
                            source_type=CONCEPT_SOURCE_TDX):
    if not records:
        return records

    validate_sort_rules(sort_rules, source_type)
    ...
    for rule in reversed(sort_rules):
        ...
        sorted_records.sort(
            key=lambda rec, f=field: rec.get("sort_values", {}).get(f),
            reverse=reverse,
        )
...
class HorizontalFieldGroup(tk.Frame):
    ...
    def build_ui(self):
        ...
        toolbar.pack(fill="x", padx=2, pady=(0, 0))

        tk.Label(
            toolbar,
            text=f"{self.title}：勾选输出，拖动☰排序",
            fg="#555555",
        ).pack(side="left")
        ...
        tk.Button(
            toolbar,
            text="全选",
            width=UI_FIELD_BUTTON_WIDTH,
            command=self.select_all,
        ).pack(side="right", padx=UI_PATH_BUTTON_PAD_X)
    ...
    def render_items(self):
        ...
        for key in self.order_keys:
            ...
            item.pack(side="left", fill="y", padx=1, pady=0)
    ...
    def refresh_colors(self):
        for item in self.items:
            ...
            if key in self.disabled_keys:
                ...
                item["handle"].configure(bg="#dddddd")
                continue
    ...
    def get_selected_in_order(self):
        return [
            key for key in self.order_keys
            if self.vars[key].get() and key not in self.disabled_keys
        ]
...
def default_output_path(config):
    ...
    concept_type = config.get("concept_source",
                              {}).get("type", CONCEPT_SOURCE_TDX)
    filename = f"{start_str}-{end_str}_{concept_type}概念板块统计排序.xlsx"
...
def build_rows_for_date(blocks, qualified_map, block_index_map,
                        block_volume_map, block_fields, detail_fields,
                        sort_rules, max_blocks, keep_all_blocks,
    ...
    block_fields = remove_tdx_only_block_fields(block_fields, concept_type)
    sort_rules = remove_tdx_only_sort_rules(sort_rules, concept_type)

    if not sort_rules:
        sort_rules = default_sort_rules_for_source(concept_type)
    ...
    for block in blocks:
        ...
        volume_data = block_volume_map.get(block_code,
                                           {}) if block_volume_map else {}
        ...
        record = {
            ...
            "sort_values": {
                ...
                "BLOCK_3D_PCT_CHG":
                block_3d_pct_chg
                if block_3d_pct_chg is not None else -999999999.0,
                "BLOCK_VOLUME":
                block_volume if block_volume != "" else -1,
                "BLOCK_VOLUME_RATIO":
                block_volume_ratio if block_volume_ratio != "" else -1.0,
                "BLOCK_AMOUNT":
                block_amount if block_amount != "" else -1.0,
                "MEMBER_COUNT":
                member_count,
                "QUALIFIED_COUNT":
                qualified_count,
    ...
    records = sort_block_rank_records(records, sort_rules, concept_type)
    ...
    for rank, rec in enumerate(records, start=1):
        ...
        if not has_detail_fields:
            ...
            for key in block_fields:
                ...
                if key in ["BLOCK_PCT_CHG", "BLOCK_3D_PCT_CHG"
                           ] and value != "" and value is not None:
                    value = float(value) / 100.0

                if key == "BLOCK_AMOUNT" and value != "" and value is not None:
                    value = float(value) / 100000000.0

                if key == "BLOCK_VOLUME" and value != "" and value is not None:
        ...
        for stock in qualified:
            ...
            for key in detail_fields:
                ...
                if key == "AMOUNT" and value != "" and value is not None:
                    value = float(value) / 100000000.0

                if key == "VOLUME" and value != "" and value is not None:
                    value = float(value) / 1000000.0
...
def style_sheet(writer, sheet_name, df, block_field_count, merge_ranges,
    ...
    for title in ["成交额", "成交额(亿)", "板块成交额", "板块成交额(亿)"]:
        col = header_map.get(title)
        if col:
            for r in range(2, ws.max_row + 1):
                ws.cell(r, col).number_format = "#,##0.00"
    ...
    for title in ["成交量", "成交量(万手)", "板块成交量", "板块成交量(万手)"]:
        col = header_map.get(title)
        if col:
            for r in range(2, ws.max_row + 1):
                ws.cell(r, col).number_format = "#,##0.00"
...
def validate_field_selection(config):
    concept_type = config.get("concept_source",
                              {}).get("type", CONCEPT_SOURCE_TDX)

    block_valid = {x["key"] for x in BLOCK_FIELD_DEFS}
    detail_valid = {x["key"] for x in DETAIL_FIELD_DEFS}

    available_block_selected = {
        x["key"]
        for x in get_available_block_field_defs(concept_type)
    }

    block_order = config["block_fields"]["order"]
    block_selected = config["block_fields"]["selected"]
    detail_order = config["detail_fields"]["order"]
    detail_selected = config["detail_fields"]["selected"]

    if set(block_order) != block_valid or len(block_order) != len(block_valid):
        raise ValueError("板块字段顺序配置错误。")

    if set(detail_order) != detail_valid or len(detail_order) != len(
            detail_valid):
        raise ValueError("明细字段顺序配置错误。")

    unknown_block = set(block_selected) - block_valid
    if unknown_block:
        raise ValueError(f"板块字段中包含未知字段：{sorted(unknown_block)}")

    unavailable_block = set(block_selected) - available_block_selected
    if unavailable_block:
        raise ValueError("当前概念源不支持以下板块指数字段，请取消选择："
                         f"{sorted(unavailable_block)}")

    unknown_detail = set(detail_selected) - detail_valid
    if unknown_detail:
        raise ValueError(f"明细字段中包含未知字段：{sorted(unknown_detail)}")

    if not block_selected and not detail_selected:
...
def validate_config(config, output_path):
    ...
    validate_field_selection(config)
    validate_sort_rules(
        config.get("sort", {}).get("rules", []),
        concept_type,
    )
...
def export_to_excel(config, output_path, log_func=print):
    ...
    if concept_type == CONCEPT_SOURCE_TDX:
        ...
        log_func("当前概念源为 THS：不读取板块指数日线；板块成交量、板块成交额使用成分股累加。")
    ...
    title_map = sort_field_title_map()
    order_map = sort_order_title_map()
    sort_desc = "；".join(f"{title_map.get(rule['field'], rule['field'])}-"
                         f"{order_map.get(rule['order'], rule['order'])}"
                         for rule in sort_rules)
    log_func(f"排序条件：{sort_desc}")
    ...
    if concept_source_supports_block_index(concept_type):
        ...
        daily_block_index = {}
        log_func("概念源为 THS，跳过板块指数日线解析；板块成交量、板块成交额将使用成分股累加。")

    log_func("开始按板块成分股汇总成交量、成交额并计算收盘量比...")
    ...
    log_func(
        f"板块成分汇总缓存日期数：{len(daily_block_volume)}，日期范围："
...
class App:
    ...
    def __init__(self, root):
        ...
        self.build_vars()
        self.build_ui()
        self.refresh_source_dependent_ui()
    ...
    def build_vars(self):
        ...
        self.sort_rules = deepcopy(
            self.config.get("sort", {}).get("rules", []))

        if not self.sort_rules:
            self.sort_rules = deepcopy(make_default_config()["sort"]["rules"])

        self.sort_field_title_var = tk.StringVar(value="板块涨幅")
    ...
    def build_ui(self):
        ...
        tk.Label(
            title_frame,
            text="概念板块统计排序导出为多页Excel表——TDX支持板块指数涨幅，THS按成分统计排序",
    ...
    def build_data_source_frame(self, parent):
        ...
        wrapper.pack(fill="x", pady=UI_SECTION_PAD_Y)
    ...
    def build_daily_source_frame(self, parent):
        ...
        tk.Entry(
            frame,
            textvariable=self.daily_tdx_root_var,
            width=UI_PATH_ENTRY_WIDTH,
        ).grid(
            row=0,
            column=1,
            sticky="we",
            padx=UI_PATH_ENTRY_PAD_X,
            pady=UI_ENTRY_PAD_Y,
        )
        ...
        tk.Button(
            ...
            text="选择文件夹",
            width=UI_BUTTON_WIDTH,
            command=self.choose_daily_tdx_root,
        ).grid(
            row=0,
            column=2,
            sticky="w",
    ...
    def build_concept_source_frame(self, parent):
        ...
        self.concept_tdx_entry = tk.Entry(
            frame,
            textvariable=self.concept_tdx_root_var,
            width=UI_PATH_ENTRY_WIDTH,
        )
        self.concept_tdx_entry.grid(
            row=0,
            column=1,
            sticky="we",
            padx=UI_PATH_ENTRY_PAD_X,
        ...
        self.concept_tdx_choose_btn.grid(
            row=0,
            column=2,
            sticky="w",
            padx=UI_PATH_BUTTON_PAD_X,
        ...
        self.concept_ths_radio = tk.Radiobutton(
            frame,
            text="THS 概念成分表",
            variable=self.concept_source_type_var,
        ...
        self.concept_ths_entry = tk.Entry(
            frame,
            textvariable=self.concept_ths_root_var,
            width=UI_PATH_ENTRY_WIDTH,
        )
        self.concept_ths_entry.grid(
            row=1,
            column=1,
            sticky="we",
            padx=UI_PATH_ENTRY_PAD_X,
        ...
        self.concept_ths_choose_btn.grid(
            row=1,
            column=2,
            sticky="w",
            padx=UI_PATH_BUTTON_PAD_X,
    ...
    def build_param_frame(self, parent):
        ...
        ).grid(row=0, column=5, sticky="w", padx=(2, 1), pady=UI_ENTRY_PAD_Y)
        ...
        ).grid(row=0, column=8, sticky="w", padx=(2, 1), pady=UI_ENTRY_PAD_Y)
        tk.Label(frame, text="%").grid(
            row=0,
            column=9,
            sticky="w",
        ...
        ).grid(row=0, column=12, sticky="w", padx=(0, 8), pady=UI_ENTRY_PAD_Y)

        tk.Label(frame, text="板块数：").grid(
            row=0,
            column=13,
            sticky="e",
            padx=(6, 2),
        ...
        ).grid(row=0, column=15, sticky="w", padx=(2, 6), pady=UI_ENTRY_PAD_Y)

        tk.Checkbutton(
            frame,
            text="不输出达标个股数为0的板块",
            variable=self.skip_zero_blocks_var,
        ).grid(row=0, column=16, sticky="w", padx=(2, 6), pady=UI_ENTRY_PAD_Y)

        self.start_date_var.trace_add(
            "write",
            lambda *_: self.update_output_path_by_dates(),
        )
        self.end_date_var.trace_add(
            "write",
            lambda *_: self.update_output_path_by_dates(),
        )
        self.concept_source_type_var.trace_add(
            "write",
            lambda *_: self.update_output_path_by_dates(),
        )
        ...
        self.mark_fill_color_var.trace_add(
            "write",
            lambda *_: self.refresh_mark_color_preview(),
        )
    ...
    def build_field_frame(self, parent):
        ...
        self.block_field_group = HorizontalFieldGroup(
            ...
            selected_keys=remove_tdx_only_block_fields(
                self.config["block_fields"]["selected"],
                self.concept_source_type_var.get(),
            ),
            on_change=self.schedule_auto_save,
            disabled_keys=self.get_current_disabled_block_field_keys(),
        )

        self.block_field_group.pack(fill="x", expand=False, padx=3, pady=0)
        ...
        self.detail_field_group.pack(fill="x", expand=False, padx=3, pady=0)

    def build_sort_log_frame(self, parent):
        container = tk.Frame(parent)
        container.pack(fill="both", expand=True, pady=UI_SECTION_PAD_Y)

        container.columnconfigure(0, weight=5, uniform="sortlog")
        container.columnconfigure(1, weight=19, uniform="sortlog")
    ...
    def build_sort_frame(self, parent):
        ...
        sort_frame.grid(row=0, column=0, sticky="nsew", padx=(0, 4))
        ...
        control_frame.grid(row=0, column=0, sticky="we", padx=4, pady=(2, 1))
        control_frame.columnconfigure(1, weight=1)

        tk.Label(control_frame, text="字段").grid(
            row=0,
            column=0,
            sticky="e",
            padx=(0, 2),
        ...
        self.sort_field_combo = ttk.Combobox(
            ...
            values=[
                item["title"]
                for item in self.get_current_available_sort_field_defs()
            ],
            width=10,
            state="readonly",
        )
        self.sort_field_combo.grid(
            row=0,
            column=1,
            sticky="we",
            padx=(0, 3),
        ...
        tk.Label(control_frame, text="方向").grid(
            row=0,
            column=2,
            sticky="e",
            padx=(0, 2),
        ...
        btn_frame.grid(row=2, column=0, sticky="we", padx=4, pady=(1, 3))
        ...
        tk.Button(
            btn_frame,
            text="上移",
            width=UI_SORT_BUTTON_WIDTH,
            command=self.move_sort_rule_up,
        ).grid(row=0, column=0, sticky="we", padx=1, pady=0)

        tk.Button(
            btn_frame,
            text="下移",
            width=UI_SORT_BUTTON_WIDTH,
            command=self.move_sort_rule_down,
        ).grid(row=0, column=1, sticky="we", padx=1, pady=0)

        tk.Button(
            btn_frame,
            text="删除",
            width=UI_SORT_BUTTON_WIDTH,
            command=self.delete_sort_rule,
        ).grid(row=0, column=2, sticky="we", padx=1, pady=0)

        tk.Button(
            btn_frame,
            text="清空",
            width=UI_SORT_BUTTON_WIDTH,
            command=self.clear_sort_rules,
        ).grid(row=0, column=3, sticky="we", padx=1, pady=0)
    ...
    def build_log_frame(self, parent):
        ...
        self.log_text = tk.Text(text_container, height=10, wrap="none")
        ...
        btn_frame.grid(row=1, column=0, sticky="e", padx=4, pady=(0, 2))
        ...
        self.export_btn.pack(side="left", padx=UI_PATH_BUTTON_PAD_X)
    ...
    def get_current_disabled_block_field_keys(self):
        source_type = self.concept_source_type_var.get()

        if concept_source_supports_block_index(source_type):
            return set()

        return set(TDX_ONLY_BLOCK_FIELD_KEYS)

    def get_current_available_sort_field_defs(self):
        return get_available_sort_field_defs(
            self.concept_source_type_var.get())

    def sanitize_sort_rules_for_current_source(self):
        source_type = self.concept_source_type_var.get()
        self.sort_rules = remove_tdx_only_sort_rules(self.sort_rules,
                                                     source_type)

        if not self.sort_rules:
            self.sort_rules = default_sort_rules_for_source(source_type)

    def refresh_sort_field_combo_values(self):
        if not hasattr(self, "sort_field_combo"):
            return

        available_defs = self.get_current_available_sort_field_defs()
        available_titles = [item["title"] for item in available_defs]
        self.sort_field_combo["values"] = available_titles

        if self.sort_field_title_var.get() not in available_titles:
            if available_titles:
                self.sort_field_title_var.set(available_titles[0])

    def refresh_source_dependent_ui(self):
        source_type = self.concept_source_type_var.get()

        if hasattr(self, "block_field_group"):
            self.block_field_group.set_disabled_keys(
                self.get_current_disabled_block_field_keys())

        self.sanitize_sort_rules_for_current_source()
        self.refresh_sort_field_combo_values()
        self.refresh_sort_listbox()

        if hasattr(self, "concept_ths_radio"):
            if source_type == CONCEPT_SOURCE_THS:
                self.concept_ths_radio.config(
                    text="THS 概念成分表（涨跌幅/开高低收不可用，成交量/成交额按成分累加）")
            else:
                self.concept_ths_radio.config(text="THS 概念成分表")
    ...
    def choose_output_file(self):
        ...
        path = filedialog.asksaveasfilename(
            ...
            initialdir=initial_dir,
            initialfile=
            f"{start_str}-{end_str}_{self.concept_source_type_var.get()}概念板块统计排序.xlsx",
            filetypes=[
                ("Excel 文件", "*.xlsx"),
                ("所有文件", "*.*"),
            ],
    ...
    def refresh_sort_listbox(self):
        if not hasattr(self, "sort_listbox"):
            return
        ...
        for idx, rule in enumerate(self.sort_rules, start=1):
            self.sort_listbox.insert(
                "end",
                f"{idx}. {title_map.get(rule['field'], rule['field'])} - "
                f"{order_map.get(rule['order'], rule['order'])}",
            )

    def add_sort_rule(self):
        try:
            field = sort_field_key_by_title(self.sort_field_title_var.get())
            order = sort_order_key_by_title(self.sort_order_title_var.get())
        except Exception as e:
            messagebox.showerror("排序条件错误", str(e))
            return
        ...
        if field not in available_fields:
            messagebox.showwarning("排序字段不可用", "当前概念成分数据源不支持该排序字段。")
            return

        if rule in self.sort_rules:
            messagebox.showwarning("排序条件已存在", "该排序条件已存在，无需重复添加。")
            return

        self.sort_rules.append(rule)
        self.refresh_sort_listbox()
        self.schedule_auto_save()
    ...
    def move_sort_rule_up(self):
        ...
        self.sort_rules[idx - 1], self.sort_rules[idx] = (
            self.sort_rules[idx],
            self.sort_rules[idx - 1],
        )
        self.refresh_sort_listbox()
    ...
    def update_output_path_by_dates(self, force_dir=None):
        ...
        source_type = self.concept_source_type_var.get() or CONCEPT_SOURCE_TDX

        self.output_path_var.set(
            win_join(output_dir,
                     f"{start_str}-{end_str}_{source_type}概念板块统计排序.xlsx"))
    ...
    def collect_config_from_ui(self):
        ...
        source_type_for_sort = self.concept_source_type_var.get()
        self.sort_rules = remove_tdx_only_sort_rules(self.sort_rules,
                                                     source_type_for_sort)

        if not self.sort_rules:
            self.sort_rules = default_sort_rules_for_source(
                source_type_for_sort)

        validate_sort_rules(self.sort_rules, source_type_for_sort)
        ...
        validate_sort_rules(config["sort"]["rules"], concept_type)
